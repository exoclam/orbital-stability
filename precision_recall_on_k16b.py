...
validation_set = np.vstack([np.array(map(float, line.split())) for line in open('kepler16.dat')])
validation_set = pd.DataFrame(validation_set,columns=['mubin','ap/abin','ebin'])
validation_set['truth'] = np.ones(100000)
...
x_validation = np.asarray(validation_set[['mubin','ap/abin','ebin']])
y_validation = np.asarray(validation_set['truth'])
...
predictions = np.asarray(zip(predictions,np.round(predictions)))
