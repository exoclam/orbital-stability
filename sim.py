...
for i in elist: # for every e
    ...
    ac_draw = np.random.uniform(0.8*acrit(i,temp_mu),1.2*acrit(i,temp_mu)) # use that pair of draws to get a
    aclist.append(ac_draw)
features = np.vstack((elist,mulist,aclist)).T # ten of each (e,mu), paired with 10 ac's per pt
...
# attach to big table
table = np.vstack((elist,mulist,aclist,output)).T 
...
np.savetxt("train.txt",table,delimiter=" ")

# for after training model
#np.savetxt("test.txt",table,delimiter=" ")
